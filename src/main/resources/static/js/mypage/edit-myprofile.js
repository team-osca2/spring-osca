...
$introInput.on('input', function() {
  ...
  // 지역 입력칸~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  const $localInput = $('.loc-input-area');
  const $locationMenus = $('.location-menu-container');
  const $locationText = $('.location-option');
  const $SelectedLoc = $('.location-option-selected');
  const $locationList = $('.location-list');
  $localInput.on('click', function() {
  $locationMenus.css('display','block');

});

$locationText.on('click', function() {
    // 모든 지역에서 클래스 초기화
    $locationList.children().addClass('location-option').removeClass('location-option-selected');
    console.log("asdfasdf")
    // 클릭된 지역에 클래스 추가
    $(this).addClass('location-option-selected').removeClass('location-option');
   
    $locationMenus.css('display','none');
    const selectedText = $(this).find('.location-text').text();
    $('.loc-singleValue').text(selectedText);

});
 

// 전문 분야~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
const $expertInputArea = $('.category-inner');
