...
public class MapperTestAnna {
    ...
    public void selectAllStudyTest(){
        adminStudyMapper.selectAllStudy().stream().map(StudyDTO::toString).forEach(log::info);
    }
//    @Test
...
//    }
    ...
    public void selectAllCafePost(){
        adminCafeAdMapper.selectAllCafePost().stream().map(CafeAdDTO::toString).forEach(log::info);
    }
